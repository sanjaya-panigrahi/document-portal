...
class DocumentIngestion:
    ...
    def __init__(self, data_dir: str = "data/document_compare", session_id: Optional[str] = None):
        """
        ...
        self.logger.info("DocHandler initialized", session_id=self.session_id, session_path=self.session_path)
    ...
    def delete_existing_files(self):
        ...
        """
        try:
            if Path(self.data_dir).exists() and Path(self.data_dir).is_dir():
                for file in Path(self.data_dir).iterdir():
                    if file.is_file():
                        file.unlink()
                        self.logger.info("Deleted file", path=str(file))
                self.logger.info("Directory Cleaned", directory = str(Path(self.data_dir)))
    ...
    def save_uploaded_files(self, reference_file, actual_file):
        ...
        try:
            ...
            self.logger.info("Existing file deleted successfully", session_id=self.session_id)
            ...
            self.logger.info("Uploaded files saved successfully", reference_file=reference_file.name, 
                             actual_file=actual_file.name, session_id=self.session_id)
        ...
        try:
            with fitz.open(pdf_path) as doc:
                ...
                for page_num in range(doc.page_count):
                    ...
                    if text.strip():
                        all_text.append(f"\n--- page {page_num} ---\n{text}")
                self.logger.info("PDF read successfully", file=pdf_path.name, session_id=self.session_id)
                return "\n".join(all_text)
        except Exception as e:
    ...
    def clean_old_sessions(self, keep_latest: int = 3):
        try:
            sessions = sorted([f for f in Path(self.data_dir).iterdir() if f.is_dir()], reverse=True)
